...
plot(x=area$land,y=area$farm,xlab="land",ylab="farm")
abline(lm1$coefficients[1],lm1$coefficients[2],col="limegreen")
#b
Tukey=function(x,y,beta,k){
  n=0
  t=0
  for (i in 1:length(x)){
    if (abs(y[i]-beta[1]-beta[2]*x[i])<=k){
      t=(y[i]-beta[1]-beta[2]*x[i])^2
    }else{
      t=2*k*abs(y[i]-beta[1]-beta[2]*x[i])-k^2
    }
    n=n+t
  }
  return(n/length(x))
  #where n is the t function, x is n in Tukey(beta0, beta1)
}

Tukey_gradient=function(x,y,beta,k){
  t=c()
  for (i in 1:length(x)){
    if (abs(y[i]-beta[1]-beta[2]*x[i])<=k){
  ...
  derivative=c()
  derivative[1]=-mean(t)
  derivative[2]=-mean(t*x)
  return(derivative)
}
derivative1 = optim(par = c(0,0),fn=Tukey,method = "Nelder-Mead", x=area$land, y=area$farm,k=190000)
abline(derivative1$par[1],derivative1$par[2],col="navy",lwd=2)

#c
derivative2 = optim(par = c(0,0),fn=Tukey,gr=Tukey_gradient, method="BFGS",x=area$land,y=area$farm,k=190000)
abline(derivative2$par[1],derivative2$par[2],col="black")

#d
derivative3 = optim(par = c(0,0),fn=Tukey,gr=Tukey_gradient, method="CG",x=area$land,y=area$farm,k=190000)
abline(derivative3$par[1],derivative3$par[2],col="coral",lty="dashed")

#e
legend("topright",legend = c("LSE","Nelder-Mead","BFGS","CG"),col =
c("limegreen","navy","black","coral"),lty = c(1,1,1,2),cex=0.8)
```

f. For which method was the value of the Tukey function the smallest?
```{r}
method<-c("Nelder-Mead","BFGS","CG")
value<-c(derivative1$value,derivative2$value,derivative3$value)
df<-data.frame(method,value)
cat(as.character(df[which(df['value']==min(df['value'])),]['method']),"has the smallest value of the Tukey function.")
```
...
When we set k=19000, it restrains the curve being effected by some outliers and growing too fast by replacing the x^2 with a linear growth.
  ...
  for(i in 1:length(t)){
    if(abs(t[i])<=19000){
      m[i] = t[i]^2
        ...
        m[i] = 2*19000*abs(t[i])-19000^2
      }
    }
  return(m)
}
curve(expr = pt,from = -100000,to = 100000)
...
out1 = optimize(FE,interval = c(0,1),Y = nhtemp)
cat("The value of beta that produces the minimum forecast error is",out1$minimum)
```
...
lines(x = c(1912:1971),y = Y.hat,col = "red")
```
...
beta2 = 0.9
for(i in 2:length(Y)){
 Y.hat1[i] = beta1*Y[i-1]+(1-beta1)*Y.hat1[i-1]
 Y.hat2[i] = beta2*Y[i-1]+(1-beta2)*Y.hat2[i-1]
}

plot(x = c(1912:1971),y = Y,type = "l",xlab = "years",ylab="temperature")
lines(x = c(1912:1971),y = Y.hat,col = "red")
lines(x = c(1912:1971),y = Y.hat1,col = "blue")
lines(x = c(1912:1971),y = Y.hat2,col = "green")
legend("topright",legend = c("original","exponential","beta = 0.1","beta =
0.9"),col = c("black","red","blue","green"),lty = c(1,1,1,1),cex=0.3)
```
...
```{r}
loglikeli = function(par,X){
 n = length(X)
 out = -n*log(1/(sqrt(2*pi)*par[2]))+sum((X-par[1])^2)/(2*par[2]^2)
 return(out)
}
out3 = optim(par = c(0,1),fn = loglikeli,method = "Nelder-Mead",X = mtcars$mpg)
n = length(mtcars$mpg)
cat(" estimated mu is ",out3$par[1],"\n estimated sigma is ",out3$par[2],"\n sample mean is ",mean(mtcars$mpg),"\n standard deviation is",sd(mtcars$mpg)*sqrt((n-1)/n))
```
...
plot(x = menarche.cases$age,y=jitter(menarche.cases$reached.menarche),xlab ="Age",ylab = "reached proportion",pch = 1)
beta1 = out4$par[1]
beta2 = out4$par[2]
